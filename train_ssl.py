...
from train_disp import cal_entropy_cos
...
def main(args):
    ...
    # align from 8 -> 20 layer of teacher
    ema.encoder_depth = 20
    ...
    for epoch in range(args.epochs):
        ...
        for raw_image, y in train_dataloader:
            ...
            with accelerator.autocast():
                ...
                time_s = torch.rand((z.shape[0], 1, 1, 1))
                time_t = torch.rand((z.shape[0], 1, 1, 1)) * args.time_teacher

                noises_s = torch.randn_like(z)
                noises_t = None

                ent_idx = torch.randperm(z.size(0), device=z.device)
                ...
                with torch.no_grad():
                    ...
                    zs = teacher_outputs["fs_tilde"] + ent_outputs["fs_tilde"]
                ...
                proj_loss = sit_outputs["proj_losses"][0].mean()
                ent_loss = sit_outputs["proj_losses"][1].mean()

                diffusion_loss = sit_loss + proj_loss * args.proj_coeff - ent_loss * args.ent_coeff
            ...
            if accelerator.sync_gradients:
                ...
                logs = {
                    ...
                    "ent_loss": accelerator.gather(ent_loss).mean().detach().item(),
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--ema-teacher", action=argparse.BooleanOptionalAction, default=False
    )
    parser.add_argument(
        "--time-teacher", type=float, default=0.2,
    )
