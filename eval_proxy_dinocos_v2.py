...
Three cosine summaries are reported at essentially no extra model cost:

1) dino_cls_cos  [PRIMARY]
   Cosine similarity between DINOv2 normalized CLS image features
   (`x_norm_clstoken`) for source and proxy images. This most literally
   implements "two DINO features -> cosine similarity".
...
also reports 1-cosine distances for all three summaries.
...
@torch.no_grad()
def extract_dino_features(encoder, x_m11, encoder_type):
    ...
    if not isinstance(feats, dict):
        raise TypeError(
            ...
            f"got {type(feats)} (encoder_type={encoder_type})"
        )

    if "x_norm_patchtokens" not in feats:
        raise KeyError(
            "DINOv2 forward_features returned no 'x_norm_patchtokens'. Keys="
            + str(list(feats.keys()))
        )
    ...
    if patch.ndim != 3:
        raise RuntimeError(
            f"Expected DINO patch tokens [B,N,C], got {tuple(patch.shape)}"
        )

    cls = feats.get("x_norm_clstoken", None)
    cls_source = "x_norm_clstoken"
    if cls is None:
        # Fallback only for encoder variants that do not expose a CLS feature.
        # The output JSON records this explicitly.
        cls = patch.mean(dim=1)
        cls_source = "mean_x_norm_patchtokens_fallback"

    if cls.ndim != 2:
        raise RuntimeError(
            f"Expected DINO CLS feature [B,C], got {tuple(cls.shape)}"
        )
...
@torch.no_grad()
def dino_cosines(source, pred):
    source_cls = source["cls"]
    pred_cls = pred["cls"]
    source_tokens = source["patch"]
    pred_tokens = pred["patch"]

    if source_cls.shape != pred_cls.shape:
        raise RuntimeError(
            f"DINO CLS shape mismatch: source={tuple(source_cls.shape)} "
            f"pred={tuple(pred_cls.shape)}"
        )
    ...
    # Primary: one image feature vs one image feature.
    cls_cos_per_image = F.cosine_similarity(
        source_cls, pred_cls, dim=-1, eps=1e-8
    )

    # Secondary: corresponding patch-token cosine, matching REPA alignment.
    ...
    # Secondary: mean-pooled patch tokens.
    ...
    return cls_cos_per_image, patch_cos_per_image, global_cos_per_image

def new_accum(device):
    return {
        "cls_sum": torch.zeros((), device=device, dtype=torch.float64),
        "cls_sumsq": torch.zeros((), device=device, dtype=torch.float64),
...
def update_accum(acc, cls_cos, patch_cos, global_cos):
    c = cls_cos.double()
    p = patch_cos.double()
    g = global_cos.double()
    acc["cls_sum"] += c.sum()
    acc["cls_sumsq"] += (c * c).sum()
    acc["patch_sum"] += p.sum()
    acc["patch_sumsq"] += (p * p).sum()
    acc["global_sum"] += g.sum()
    acc["global_sumsq"] += (g * g).sum()
    acc["count"] += float(c.numel())
...
def main(args):
    ...
    if rank == 0:
        ...
        print("DINO features        : x_norm_clstoken + x_norm_patchtokens", flush=True)
        print("primary metric       : CLS cosine", flush=True)
    ...
    with torch.no_grad():
        for img, y, _name in pbar:
            ...
            src_dino = extract_dino_features(
                dino, img, dino_type
            )
            ...
            for kind, t in settings:
                ...
                pred_dino = extract_dino_features(
                    dino, pred_raw, dino_type
                )
                cls_cos, patch_cos, global_cos = dino_cosines(
                    src_dino, pred_dino
                )
                update_accum(
                    accum[key], cls_cos, patch_cos, global_cos
                )

                if kind != "recon":
                    del z0_hat_norm, z0_hat_raw
                del pred_raw, pred_dino, cls_cos, patch_cos, global_cos
    ...
    if rank == 0:
        ...
        for kind, t in settings:
            ...
            cls_mean, cls_std = mean_std(
                a["cls_sum"], a["cls_sumsq"], n
            )
            ...
            metrics[key] = {
                "dino_cls_cos": cls_mean,
                "dino_cls_cos_std": cls_std,
                "dino_cls_dist": 1.0 - cls_mean,
        ...
        result = {
            ...
            "dino": {
                ...
                "features": "x_norm_clstoken + x_norm_patchtokens",
                "cls_source": "x_norm_clstoken if exposed, otherwise recorded fallback",
                "preprocess": (
                    "[-1,1] -> clamp [0,1] -> ImageNet mean/std "
                    "-> bicubic 224"
                ),
                "primary": "x_norm_clstoken_cosine",
            },
            "metrics": metrics,
        }
        ...
        for kind, t in settings:
            ...
            print(
                f"{key:16s}  "
                f"DINO_cls_cos={m['dino_cls_cos']:.8f}  "
