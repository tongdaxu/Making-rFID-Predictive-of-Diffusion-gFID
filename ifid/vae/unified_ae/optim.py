...
def _as_tuple(values: Any, length: int = 2) -> tuple:
    if isinstance(values, (list, tuple)):
        if len(values) != length:
            raise ValueError(
                f"Expected sequence of length {length}, got {len(values)}."
            )
...
def build_optimizer(
    ...
    if opt_type != "adamw":
        raise ValueError(
            f"Unsupported optimizer '{opt_type}'. Only AdamW is currently available."
        )
...
def build_scheduler(
    ...
    base_lr = float(
        sched_cfg.get(
            "base_lr", training_cfg.get("base_lr", optimizer.param_groups[0]["lr"])
        )
    )
    ...
    if warmup_steps_cfg is not None:
        ...
        warmup_epochs = float(
            sched_cfg.get("warmup_epochs", training_cfg.get("decay_start_epoch", 0))
        )
    ...
    if decay_end_steps_cfg is not None:
        ...
        decay_end_epoch = float(
            sched_cfg.get(
                ...
                training_cfg.get(
                    "decay_end_epoch",
                    warmup_steps / steps_per_epoch if steps_per_epoch else 0,
                ),
            )
        )
    ...
    if schedule_type == "linear":
        ...
        raise ValueError(
            f"Unsupported scheduler '{schedule_type}'. Choose from ['linear', 'cosine']."
        )
