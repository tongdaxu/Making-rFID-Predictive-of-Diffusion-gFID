...
from ifid.vae.dinov3.layers import LayerScale, Mlp, PatchEmbed, RMSNorm, RopePositionEmbedding, SelfAttentionBlock, SwiGLUFFN
from ifid.vae.dinov3.utils import named_apply
...
class DINOv3VAE(nn.Module):
    def __init__(self, ckpt_path, *args, **kwargs):
        ...
        vae_ckpt = torch.load(ckpt_path, map_location="cpu")["teacher"]
        miss_keys, unexp_keys = self.load_state_dict(vae_ckpt, strict=False)
        print("missing: ", miss_keys)
        print("unexp_keys: ", unexp_keys)

    def encode(self, x):
        backbone_out = self.backbone(x, is_training=True)
        z = backbone_out["x_norm_patchtokens"].permute(0, 2, 1).reshape(-1, self.embed_dim, 16, 16)
        return z
...
if __name__ == "__main__":
    ...
    configs = OmegaConf.load("/mnt/task_runtime/configs/DINOv3VITL.yaml")
