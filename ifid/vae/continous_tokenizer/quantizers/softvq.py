...
class SoftVectorQuantizer(nn.Module):
    ...
    def forward(self, z):
        ...
        embedding = (
            F.normalize(self.embedding, p=2, dim=-1) if self.l2_norm else self.embedding
        )
        if self.l2_norm:
            z_segments = F.normalize(z_segments, p=2, dim=-1)

        z_flat = (
            z_segments.permute(1, 0, 2, 3)
            .contiguous()
            .view(self.num_codebooks, -1, self.e_dim)
        )
        ...
        # Reshape back
        z_q = (
            z_q.view(self.num_codebooks, batch_size, segment_length, self.e_dim)
            .permute(1, 0, 2, 3)
            .contiguous()
        )
        ...
        with torch.no_grad():
            zq_z_cos = F.cosine_similarity(
                z_segments.view(-1, self.e_dim), z_q.view(-1, self.e_dim), dim=-1
            ).mean()
        ...
        if self.show_usage and self.training:
            for k in range(self.num_codebooks):
                ...
                self.codebook_used[k, :-cur_len].copy_(
                    self.codebook_used[k, cur_len:].clone()
                )
