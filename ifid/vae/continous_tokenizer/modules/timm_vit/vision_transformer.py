...
from ifid.vae.continous_tokenizer.modules.timm_vit.rope_utils import apply_rotary_emb
        ...
        if self.fused_attn:
            ...
            if freqs_cis is not None:
                if num_latent_tokens == 0:
                    q[:, :, num_prefix_tokens:, :], k[:, :, num_prefix_tokens:] = (
                        apply_rotary_emb(
                            q[:, :, num_prefix_tokens:],
                            k[:, :, num_prefix_tokens:],
                            freqs_cis=freqs_cis,
                    ...
                    (
                        q[:, :, num_prefix_tokens:-num_latent_tokens, :],
                        k[:, :, num_prefix_tokens:-num_latent_tokens],
                    ) = apply_rotary_emb(
                        q[:, :, num_prefix_tokens:-num_latent_tokens],
                        k[:, :, num_prefix_tokens:-num_latent_tokens],
                        freqs_cis=freqs_cis,
...
class MoVQBlockv2(nn.Module):
    def __init__(
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(), nn.Linear(latent_dim, 4 * dim, bias=True)
        )

    def forward(
        self,
        x: torch.Tensor,
        interpolate_zq: torch.Tensor,
        ...
        attn_out = self.attn(
            attn_out, attn_mask, freqs_cis, num_prefix_tokens, num_latent_tokens
        )
...
class MoVQNorm(nn.Module):
    def __init__(self, latent_dim, dim):
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(), nn.Linear(latent_dim, 2 * dim, bias=True)
        )
...
class VisionTransformer(nn.Module):
    ...
    def forward_intermediates(
        ...
        if self.dynamic_img_size:
            x = x.reshape(
                B, height // self.patch_size, width // self.patch_size, x.shape[-1]
            )
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
...
@register_model
def vit_tinytiny_patch14_dinov2_movq2(
...
@register_model
def vit_small_patch14_dinov2_movqv2(
    pretrained: bool = False, **kwargs
) -> VisionTransformer:
    """ViT-S/14 for DINOv2"""
    model_args = dict(
...
@register_model
def vit_base_patch14_dinov2_movqv2(
    pretrained: bool = False, **kwargs
) -> VisionTransformer:
    """ViT-B/14 for DINOv2"""
    model_args = dict(
