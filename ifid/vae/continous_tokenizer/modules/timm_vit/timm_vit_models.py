...
from ifid.vae.continous_tokenizer.modules.timm_vit.to_pixel import ToPixel
from ifid.vae.continous_tokenizer.modules.timm_vit.vision_transformer import (
    MoVQNorm,
    MoVQBlockv2,
)
from ifid.vae.continous_tokenizer.modules.timm_vit.rope_utils import (
    compute_axial_cis,
    compute_mixed_cis,
...
class TimmViTEncoder(nn.Module):
    def __init__(
        ...
        if self.rope_mixed and self.use_rope:
            ...
            if base_img_size != model_kwargs["img_size"]:
                t_x, t_y = init_t_xy(
                    end_x=base_img_size // model_kwargs["patch_size"],
                    end_y=base_img_size // model_kwargs["patch_size"],
                )
            else:
                t_x, t_y = init_t_xy(
                    end_x=model_kwargs["img_size"] // model_kwargs["patch_size"],
                    end_y=model_kwargs["img_size"] // model_kwargs["patch_size"],
                )
    ...
    def forward(self, x, return_mask=False):
        ...
        if "eva02" not in self.model_name:
