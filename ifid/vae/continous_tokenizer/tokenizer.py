...
from ifid.vae.continous_tokenizer.modules import Encoder, Decoder, TimmViTEncoder, TimmViTDecoder
...
def bits_per_dim(logits, logdet, dim_count, pdf=None):
    ...
    nll = torch.sum(
        nll + torch.log(torch.tensor(127.5, device=nll.device)),
        dim=tuple(range(1, nll.dim()))
    )
    nll = nll.unsqueeze(-1)

    bits = nll - logdet
    normalizer = torch.log(torch.tensor(2, device=nll.device)) * dim_count
...
class VQModel(nn.Module, PyTorchModelHubMixin):
    def __init__(self, config: ModelArgs, 
        ...
        if config.repa and (config.enc_type == 'vit' or config.enc_type =='flow') :
            ...
            from cont.modelling.lpips.lpips_timm import Normalize, Denormalize
        ...
        elif config.enc_type == 'flow':
            self.encoder = Jet(
                patch_size=16, patch_dim=768, n_patches=256, img_dims=(256, 256),
                token_drop=config.enc_token_drop,
                token_drop_max=config.enc_token_drop_max,
            )
            self.quant_conv = nn.Linear(768, config.codebook_embed_dim)
...
class FlowModel(VQModel):
    ...
    def decode(self, quant, x=None, h=None, w=None):
        return self.encoder.inverse(quant)[0]
...
class MaskAEModel(AEModel):
    def __init__(self, config: ModelArgs,
        ...
        if self.aux_hog_decoder:
            from cont.utils.hog import HOGGenerator
    ...
    def encode(self, x):
        ...
        if self.enc_type == 'flow':
            # dequant step
            noise = (torch.rand_like(x) - 0.5) / 128
            x = x + noise
        ...
        if self.training:
            if self.enc_type == 'flow':
                h, mask, logdet = self.encoder(x, return_mask=True)
                dim_count = np.prod(x[0].shape)
                bpd, nll, logdet = bits_per_dim(h, logdet, dim_count)
                info['bpd'] = bpd
            else:
                h, mask = self.encoder(x, return_mask=True)
        else:
            if self.enc_type == 'flow':
                h, logdet = self.encoder(x)
            else:
                h = self.encoder(x)
        quant = self.quant_conv(h)
        emb_loss = (torch.tensor(0.), torch.tensor(0.), torch.tensor(0.), torch.tensor(0.))
