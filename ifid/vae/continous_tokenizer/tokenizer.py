...
from ifid.vae.continous_tokenizer.modules import (
    Encoder,
    Decoder,
    TimmViTEncoder,
    TimmViTDecoder,
)
from ifid.vae.continous_tokenizer.quantizers.vq import VectorQuantizer
from ifid.vae.continous_tokenizer.quantizers.kl import DiagonalGaussianDistribution
...
class VQModel(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        if config.repa and config.enc_type == "vit":
            ...
            self.projection = build_mlp(
                config.codebook_embed_dim, config.repa_proj_dim, repa_z_dim
            )
        ...
        if config.enc_type == "cnn":
            ...
            self.quant_conv = nn.Linear(
                self.encoder.embed_dim, config.codebook_embed_dim
            )
        ...
        if config.dec_type == "cnn":
            ...
            self.post_quant_conv = nn.Linear(
                config.codebook_embed_dim, self.decoder.embed_dim
            )
        # check movq
    ...
    def encode(self, x):
        ...
        if self.repa and self.training:
            ...
            z = self.repa_model.forward_features(rescale_x)[
                :, self.repa_model.num_prefix_tokens :
            ]
            ...
            if self.repa_align == "global":
                ...
                z = F.adaptive_avg_pool1d(z.permute(0, 2, 1), quant.shape[1]).permute(
                    0, 2, 1
                )
                z_hat = quant
                z_hat = self.projection(z_hat)
                ...
                z = F.adaptive_avg_pool1d(
                    z[:, indices, :].permute(0, 2, 1), quant.shape[1]
                ).permute(0, 2, 1)
                z_hat = quant
                z_hat = self.projection(z_hat)
    ...
    def __init__(self, config: ModelArgs):
        ...
        if config.enc_type == "cnn":
            self.quant_conv = nn.Conv2d(
                config.z_channels, config.codebook_embed_dim * 2, 1
            )
        elif config.enc_type == "vit":
            self.quant_conv = nn.Linear(
                self.encoder.embed_dim, config.codebook_embed_dim * 2
            )
...
class MaskAEModel(AEModel):
    def __init__(
        ...
        if self.aux_hog_decoder:
            ...
            self.post_quant_conv_hog = nn.Linear(
                config.codebook_embed_dim, self.decoder_hog.embed_dim
            )
        ...
        if self.aux_dino_decoder:
            ...
            self.decoder_dino = TimmViTDecoder(
                ...
                model_name=config.aux_dec_model,
                model_kwargs={
                    "img_size": self.repa_model.img_size,
            ...
            self.post_quant_conv_dino = nn.Linear(
                config.codebook_embed_dim, self.decoder_dino.embed_dim
            )
            self.to_pixel_dino = nn.Linear(
                self.decoder_dino.embed_dim, self.repa_model.embed_dim
            )
            if "movq" in config.aux_dec_model:
        ...
        if self.aux_clip_decoder:
            ...
            self.decoder_clip = TimmViTDecoder(
                ...
                model_name=config.aux_dec_model,
                model_kwargs={
                    "img_size": self.clip_model.img_size,
            ...
            self.post_quant_conv_clip = nn.Linear(
                config.codebook_embed_dim, self.decoder_clip.embed_dim
            )
            self.to_pixel_clip = nn.Linear(
                self.decoder_clip.embed_dim, self.clip_model.embed_dim
            )
            if "movq" in config.aux_dec_model:
        ...
        if self.training:
            ...
            if self.aux_hog_decoder:
                ...
                if self.aux_loss_mask:
                    ...
                    hog_rec_loss = (
                        (hog_rec_loss * mask).sum() / mask.sum() / z_hog.size(-1)
                    )
                else:
            ...
            if self.aux_dino_decoder:
                ...
                z_dino = self.repa_model.forward_features(rescale_x)[
                    :, self.repa_model.num_prefix_tokens :
                ]
            ...
            if self.aux_clip_decoder:
                ...
                z_clip = self.clip_model.forward_features(rescale_x)[
                    :, self.clip_model.num_prefix_tokens :
                ]
