        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            if heun and (i < num_steps - 1):
                ...
                if cfg_scale > 1.0 and t_cur <= guidance_high and t_cur >= guidance_low:
                    ...
                    d_prime = d_prime_uncond + cfg_scale * (
                        d_prime_cond - d_prime_uncond
                    )
...
def euler_maruyama_sampler(
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-2], t_steps[1:-1])):
            ...
            s_cur = get_score_from_velocity(
                v_cur, model_input, time_input, path_type=path_type
            )
    ...
    # compute drift
    v_cur = model(model_input.to(dtype=_dtype), time_input.to(dtype=_dtype), **kwargs)[
        0
    ].to(torch.float64)
