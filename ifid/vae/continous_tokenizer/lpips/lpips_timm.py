...
class NetLinLayer(nn.Module):
    ...
    def __init__(self, chn_in, chn_out=1, use_dropout=False, use_linear=False):
        ...
        if use_linear:
            layers += [
                nn.Linear(chn_in, chn_out, bias=False),
            ]
        else:
            layers += [
    ...
    def __init__(
        ...
        if "vit" in model_name:
            self.model = create_model(
                model_name, pretrained=True, img_size=256, patch_size=16
            )
        else:
        ...
        if "resnet50" in model_name:
            ...
            or "vit_small_patch14_reg4_dinov2.lvd142m"
            or "vit_base_patch14_reg4_dinov2.lvd142m"
        ):
            if self.intermediate_loss:
                ...
                if dino_variants == "depth12_no_train":
                    ...
                    self.lins = nn.ModuleList(
                        [
                            NetLinLayer(self.model.embed_dim, use_dropout=use_dropout)
                            for _ in self.key_depths
                        ]
                    )
            self.image_size = 256
            ...
            if model_name == "vit_small_patch14_dinov2.lvd142m":
                if dino_variants == "depth12":
                    state_dict = torch.load(
                        "tokenizer/cache/dino2s_i256p16_layers12/latest.pth",
                        map_location="cpu",
                    )
                elif dino_variants == "depth4":
                    state_dict = torch.load(
                        "tokenizer/cache/dino2s_i256p16_layers4/latest.pth",
                        map_location="cpu",
                    )
                elif dino_variants == "depth6":
                    state_dict = torch.load(
                        "tokenizer/cache/dino2s_i256p16_layers6/latest.pth",
                        map_location="cpu",
                    )
            elif model_name == "vit_base_patch14_reg4_dinov2.lvd142m":
                if dino_variants == "depth12":
                    state_dict = torch.load(
                        "checkpoints/dino2b_i256p16_layers12/latest.pth",
                        map_location="cpu",
                    )
            self.load_state_dict(self.get_lin_state_dict(state_dict), strict=False)
        ...
        if self.intermediate_loss:
            if self.key_depths is not None:
                for i in self.key_depths:
                    diff = (
                        normalize_tensor(outs0_feats[i])
                        - normalize_tensor(outs1_feats[i])
                    ) ** 2
                    diffs[diff_cnt] = diff
                    diff_cnt += 1
            else:
