...
def latent_to_feature(
    ...
    """
    Convert a VAE latent into a feature vector for linear probing.
    ...
    feature_type:
      flatten: [B, ...] -> [B, prod(...)]
      gap:     spatial/token global average -> [B, C]
      pool:    [B,C,H,W] -> adaptive_avg_pool2d(pool_size) -> flatten
    """
    ...
    if feature_type == "flatten":
        ...
        if z.ndim == 4:
            ...
            feat = z.mean(dim=1)  # token latent [B,N,C]
        elif z.ndim == 2:
            feat = z
        else:
            raise RuntimeError(f"Unsupported latent shape for gap: {tuple(z.shape)}")
...
def main(args):
    ...
    if args.max_feature_dim > 0 and feature_dim > args.max_feature_dim:
        raise RuntimeError(
            ...
            f"Use --feature-type gap or --feature-type pool --pool-size N."
        )
...
def parse_args():
    ...
    parser.add_argument("--feature-type", type=str, default="flatten", choices=["flatten", "gap", "pool"])
