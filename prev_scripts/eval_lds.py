...
# LDS
# -------------------------
def _default_radii(H: int, W: int):
    """
    Teacher's original code uses r_near = H // 2 and r_far = H // 2.

    For real 2D maps like 32x32, this matches exactly.
    For 1D fallback maps like 1xL, H//2 = 0 would make near_mask empty,
    so use max(H, W)//4 as a fallback.
    """
    if H > 1 and W > 1:
        r = max(1, H // 2)
    else:
...
@torch.no_grad()
def compute_lds_with_details(
    x: torch.Tensor,
    r_near: int | None = None,
    r_far: int | None = None,
    ...
    if r_near is None or r_far is None:
        rn, rf = _default_radii(H, W)
        if r_near is None:
            r_near = rn
        if r_far is None:
            r_far = rf

    feats = x.flatten(2).transpose(1, 2)  # B,T,C
    feats = F.normalize(feats.float(), dim=-1, eps=1e-8)

    sim = feats @ feats.transpose(-1, -2)  # B,T,T
    ...
    if dist_type == "manhattan":
        ...
        dist = torch.sqrt(((coords[:, None, :] - coords[None, :, :]).float() ** 2).sum(-1))
    else:
        raise ValueError(f"Unknown dist_type: {dist_type}")

    eye = torch.eye(T, dtype=torch.bool, device=x.device)

    near_mask = (dist < r_near) & (~eye)
    far_mask = dist >= r_far
    ...
    near_mean = sim[:, near_mask].mean(dim=1)
    far_mean = sim[:, far_mask].mean(dim=1)
    lds = near_mean - far_mean

    return lds, near_mean, far_mean
...
def evaluate_lds(args):
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            ...
            lds, near_mean, far_mean = compute_lds_with_details(
                z,
                r_near=args.r_near,
                r_far=args.r_far,
...
def build_parser():
    ...
    p.add_argument("--r_near", "--r-near", dest="r_near", type=int, default=None)
    p.add_argument("--r_far", "--r-far", dest="r_far", type=int, default=None)
