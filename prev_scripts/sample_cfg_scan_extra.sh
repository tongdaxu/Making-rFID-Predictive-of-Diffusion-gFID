...
if (( ${#EXPS[@]} == 0 )); then
    ...
    echo "Usage:"
    echo "  bash sample_cfg_scan_extra.sh /path/to/exp1 /path/to/exp2"
    exit 1
fi
...
update_vae_config_for_exps() {
    python3 - "$CONFIG_DIR" "${EXPS[@]}" <<'PY'
import json
import shutil
import sys
from pathlib import Path
...
for exp_arg in sys.argv[2:]:
    exp = Path(exp_arg)
    name = exp.name.lower()

    args_path = exp / "args.json"
    ckpt_path = exp / "checkpoints" / "0400000.pt"

    if not args_path.exists():
        print(f"[WARN no args.json] {exp}")
        continue

    if not ckpt_path.exists():
        print(f"[WARN no checkpoint] {exp} -> {ckpt_path}")
        continue
    ...
    if yaml_path is None:
        print(f"[WARN no yaml rule] {exp.name}; keep existing args.json vae_config")
        continue

    if not yaml_path.exists():
        print(f"[WARN missing yaml] {exp.name} -> {yaml_path}; keep existing args.json vae_config")
        continue
...
run_one_cfg() {
    ...
    local candidate_log
    shopt -s nullglob
    for candidate_log in "${log_file}" "${LOG_DIR}"/part*_"${exp_name}"_cfg"${cfg}".log "${LOG_DIR}"/extra_"${exp_name}"_cfg"${cfg}".log; do
...
for EXP in "${EXPS[@]}"; do
    if [[ ! -d "${EXP}" ]]; then
        echo "[WARN] EXP dir missing, skip: ${EXP}"
        continue
    fi
