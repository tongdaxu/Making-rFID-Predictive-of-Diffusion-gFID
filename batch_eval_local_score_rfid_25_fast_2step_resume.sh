...
SCRIPT="${SCRIPT:-eval_local_score_rfid_25_fast_2step.py}"
...
OUT_TAG="${OUT_TAG:-lsm2_fast_bs64_p1_t06_03_t08_04_50k50k_k${CANDIDATE_TOPK}}"
OUT_ROOT="${OUT_ROOT:-/share/xutongda/REPA-E/exps_interpolate/local_score_rfid_25/${OUT_TAG}}"

# Reuse the exact reference caches already built by the single-step LSM runs.
...
ref_bs_for_config() {
  ...
  case "${cfg}" in
    "RAE.yaml"|"UAE.yaml"|"ScaleRAE_SIGLIP2_NONORM.yaml"|"ScaleRAE_WEBSSL.yaml")
      echo "${HIGH_DIM_REF_BS:-128}" ;;
    "SVG.yaml"|"SVGT2I_P_STAGE1_256.yaml"|"FLUX2VAE.yaml"|"PAE_DINOv2L_d32.yaml")
      echo "${MID_DIM_REF_BS:-128}" ;;
    *)
...
query_block_for_config() {
  ...
  case "${cfg}" in
    "RAE.yaml"|"UAE.yaml"|"ScaleRAE_SIGLIP2_NONORM.yaml"|"ScaleRAE_WEBSSL.yaml")
      echo "${HIGH_DIM_QUERY_BLOCK:-8}" ;;
    "SVG.yaml"|"SVGT2I_P_STAGE1_256.yaml"|"FLUX2VAE.yaml"|"PAE_DINOv2L_d32.yaml")
      echo "${MID_DIM_QUERY_BLOCK:-8}" ;;
    *)
      echo "${DEFAULT_QUERY_BLOCK:-8}" ;;
  esac
}
...
launch_job() {
  ...
  echo "Launching ${cfg} on GPU ${gpu}: query_bs=${bs}, ref_bs=${refbs}, qblock=${qblock}, query_imgs=$((bs*qblock)), cand_chunk=${cchunk}, trajectories=0.6->0.3->0 / 0.8->0.4->0"
