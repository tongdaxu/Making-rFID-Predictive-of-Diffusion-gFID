#!/usr/bin/env bash
# A800-80GB exact mode: exhaustive full-hidden NN over all 50k references.
...
PY_SCRIPT="${REPO}/eval_sit_hidden_ifid_exact_fullnn_v7_a800.py"
...
CACHE_ROOT="${OUT_ROOT}/hidden_ifid_exact_fullnn_cache_50k50k"
RESULT_ROOT="${OUT_ROOT}/hidden_ifid_exact_fullnn_results_50k50k"
LOG_ROOT="${OUT_ROOT}/logs_hidden_ifid_exact_fullnn_50k50k"
STATUS_ROOT="${OUT_ROOT}/status_hidden_ifid_exact_fullnn_50k50k"
...
T_VALUE=0.1
ALPHA=0.5
EXACT_REF_CHUNK="${EXACT_REF_CHUNK:-4096}"
SEED=0
...
run_gpu_worker() {
  ...
  for ((idx=gpu; idx<N; idx+=4)); do
    ...
    touch "${STATUS_ROOT}/${tag}.running"
    {
      ...
      echo "batch_size=${bs}"
      echo "reference=${SMALL_REF} validation=${SMALL_VAL} exact_full_nn=1 exact_ref_chunk=${EXACT_REF_CHUNK}"
      echo "gpu_reference_preload=1 gpu_memory_reserve_gib=${GPU_MEMORY_RESERVE_GIB} prefetch_factor=${PREFETCH_FACTOR}"
      echo "============================================================"
    } > "$model_log"
    ...
    env \
      ...
      python -u "$PY_SCRIPT" \
        ...
        --alpha "$ALPHA" \
        --nn-metric cosine \
        --exact-ref-chunk "$EXACT_REF_CHUNK" \
        --seed "$SEED" \
        --device cuda:0 \
        --expected-tokens 0 \
        --expected-dim 1152 \
