    ...
    for epoch in range(args.epochs):
        ...
        for raw_image, y in train_dataloader:
            ...
            with accelerator.autocast():
                ...
                with torch.no_grad():
                    z_plus = random_spatial_mask(z).clone()
                    time_input, noises = sit_outputs["time_input"].clone(), sit_outputs["noises"].clone()
                    time_input[1:] = time_input[0:1]
                    noises[1:] = noises[0:1]

                    sit_outputs_aug = model(
                        x=torch.cat([z.clone(), z_plus], dim=0),
                    ...
                    fs_minus, fs_plus = sit_outputs_aug["fs_tilde"][0].chunk(2, dim=0)
                    align_cos = cal_align_cos(fs_minus, fs_plus).mean()
                    entropy_cos = cal_entropy_cos(fs_minus).mean()
            ...
            if accelerator.sync_gradients:
                ...
                logs = {
                    ...
                    "align_cos": accelerator.gather(align_cos).mean().detach().item(),
                    "entropy_cos": accelerator.gather(entropy_cos).mean().detach().item(),
