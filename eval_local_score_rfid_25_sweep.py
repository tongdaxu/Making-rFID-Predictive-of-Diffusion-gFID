...
Default P-sweep settings:
    raw t=0.2, P in {1,3,5,7}
    raw t=0.4, P in {1,3,5,7}
...
def parse_args():
    ...
    p.add_argument("--bs", type=int, default=4, help="Validation/query batch size.")
    p.add_argument("--ref-bs", type=int, default=64, help="Reference-bank VAE encoding batch size; independent of query --bs.")
    ...
    p.add_argument(
        ...
        default=[
            ...
            (0.4, 1), (0.4, 3), (0.4, 5), (0.4, 7),
        ],
        help="Local-Score settings t:P. Sweep default: t=0.2/0.4 x P=1/3/5/7.",
    )
    p.add_argument(
        "--time-mode",
...
@torch.no_grad()
def local_score_mu_topk_multi_p(
    z_t: torch.Tensor,
    candidate_native_cpu: torch.Tensor,
    mean: torch.Tensor,
    std: torch.Tensor,
    ...
    """Compute several P values for the same t in one candidate pass."""
    patch_sizes = [int(p) for p in patch_sizes]
    if len(set(patch_sizes)) != len(patch_sizes):
        raise ValueError(f"Duplicate patch sizes: {patch_sizes}")
    ...
    running_max = {
        p: torch.full((B, H, W), -float("inf"), device=device, dtype=torch.float32)
        for p in patch_sizes
    }
    running_den = {
        p: torch.zeros((B, H, W), device=device, dtype=torch.float32)
        for p in patch_sizes
    }
    running_num = {
        p: torch.zeros((B, C, H, W), device=device, dtype=torch.float32)
        for p in patch_sizes
    }
    ...
    for s in range(0, K, candidate_chunk_size):
        ...
        # Shared across P=1/3/5/7 at this t.
        diff2 = ((float(alpha) * cand_sp - z_t[:, None]) ** 2).sum(dim=2)

        for p in patch_sizes:
            local_dist = box_sum_2d(diff2, p)
            logits = -local_dist / denom_const

            chunk_max = logits.max(dim=1).values
            new_max = torch.maximum(running_max[p], chunk_max)
            old_scale = torch.exp(running_max[p] - new_max)
            old_scale = torch.where(
                torch.isfinite(running_max[p]), old_scale, torch.zeros_like(old_scale)
            )
            w = torch.exp(logits - new_max[:, None])

            running_den[p] = running_den[p] * old_scale + w.sum(dim=1)
            running_num[p] = (
                running_num[p] * old_scale[:, None]
                + (w[:, :, None] * cand_sp).sum(dim=1)
            )
            running_max[p] = new_max

        del cand_native, cand_norm_native, cand_sp, diff2

    return {
        p: running_num[p] / running_den[p].clamp_min(1e-20)[:, None]
        for p in patch_sizes
    }
...
def main(args):
    ...
    for meta in setting_meta:
        settings_by_t.setdefault(meta["t_raw"], []).append(meta)

    print("=" * 72)
    print("Local-Score rFID P-sweep (25-VAE x 1000-query pilot)")
    ...
    print("ref_bs / query_bs:", args.ref_bs, "/", args.bs)
    ...
    while done < total_batches:
        ...
        for rec, cand_cpu in zip(query_records, candidate_cpu_batches):
            ...
            # Group by t so P=1/3/5/7 share z_t, candidate transfer,
            # normalization and per-candidate diff2.
            for t_raw, metas in settings_by_t.items():
                alpha, sigma = metas[0]["alpha"], metas[0]["sigma"]
                z_t = alpha * z_norm_sp + sigma * eps
                ps = [m["patch_size"] for m in metas]

                mu_by_p = local_score_mu_topk_multi_p(
                    z_t=z_t,
                ...
                for meta in metas:
                    mu_sp = mu_by_p[meta["patch_size"]]
                    mu_native_norm = spatial_to_native(
                        mu_sp, native_shape=native_shape, token_grid=args.token_grid
                    )
                    mu_native_raw = denormalize_native(
                        mu_native_norm, mean_d, std_d
                    )
                    ...
                    del mu_native_norm, mu_native_raw, img_hat, img_hat01
    ...
    result = {
        ...
        "ref_batch_size": int(args.ref_bs),
        "query_batch_size": int(args.bs),
        ...
        "sweep_note": "P sweep is a pilot hyperparameter study; choose P by cross-VAE correlation with gFID, not by minimum LS-rFID on one VAE.",
