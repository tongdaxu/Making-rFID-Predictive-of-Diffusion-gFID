#!/usr/bin/env python3
"""
Exact same-position local-NN iFID, P=1, for the 25-VAE benchmark.
...
For the default slerp mode, d is cosine distance, matching the original iFID
nearest-neighbor search convention for slerp. For linear interpolation, d is
squared Euclidean distance.

After selecting the hard top-1 nearest neighbor independently at every position,
interpolation is also local:

    z_hat(x) = slerp(z(x), z_j*(x)(x), alpha)

with alpha=0.5 by default.

This script reuses the raw reference-latent cache format from
eval_local_score_rfid_25_fast.py. The default ref_size=50000 therefore directly
reuses the cache built for the recent LSM runs.

For native token latents [B,N,C], P=1 "same position" means same token index n.
The temporary HxW reshape is only a bijective layout convenience and does not
introduce spatial-neighborhood semantics at P=1.
"""
...
def local_slerp_vectors(q: torch.Tensor, n: torch.Tensor, alpha: float, eps=1e-7):
    """
    q, n: [..., C].
    SLERP is performed independently for every local P=1 vector.
    """
    qf = q.float()
    nf = n.float()

    qn = qf / qf.norm(dim=-1, keepdim=True).clamp_min(eps)
    nn = nf / nf.norm(dim=-1, keepdim=True).clamp_min(eps)

    dot = (qn * nn).sum(dim=-1).clamp(-1.0, 1.0)
    theta = torch.acos(dot)
    sin_theta = torch.sin(theta)

    wq = torch.sin((1.0 - alpha) * theta) / sin_theta.clamp_min(eps)
    wn = torch.sin(alpha * theta) / sin_theta.clamp_min(eps)

    out = wq[..., None] * qf + wn[..., None] * nf

    small = sin_theta.abs() < 1e-6
    if small.any():
        lin = (1.0 - alpha) * qf + alpha * nf
        out = torch.where(small[..., None], lin, out)
...
def local_interpolate_p1(
    ...
    q_sp = native_to_spatial(query_native.float(), token_grid=token_grid)
    n_sp = native_to_spatial(nn_native.float(), token_grid=token_grid)

    B, C, H, W = q_sp.shape
    q = q_sp.permute(0, 2, 3, 1).reshape(B, H * W, C)
    n = n_sp.permute(0, 2, 3, 1).reshape(B, H * W, C)

    if intp == "linear":
        z = (1.0 - alpha) * q + alpha * n
    elif intp == "slerp":
        z = local_slerp_vectors(q, n, alpha)
    else:
        raise ValueError(intp)

    z_sp = z.reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous()
    return spatial_to_native(
        z_sp,
        native_shape=native_shape,
        token_grid=token_grid,
    )
...
def main(args):
    ...
    print("=" * 80)
    print("Exact same-position local-NN iFID, P=1")
    ...
    for img, y, names in tqdm(
        val_loader,
        desc=f"local-iFID-p1 {args.exp_name}",
    ):
        ...
        if preview_left > 0:
            ...
            for j in range(nsave):
                ...
                save_tensor(
                    out_cpu[j],
                    preview_dir / f"{stem}_local_ifid_p1.png",
                )
            preview_left -= nsave
    ...
    metric_key = f"local_ifid_p1_{args.intp}_a{args.alpha:g}"
