#!/usr/bin/env bash
# V6-aligned L2 mode on A800-80GB: L2 sketch retrieval + exact full-hidden L2 reranking.
...
PY_SCRIPT="${REPO}/eval_sit_hidden_ifid_fullsketch_l2_v6_a800.py"
...
CACHE_ROOT="${OUT_ROOT}/hidden_ifid_fullsketch_cache_50k50k"
RESULT_ROOT="${OUT_ROOT}/hidden_ifid_fullsketch_l2_results_50k50k"
LOG_ROOT="${OUT_ROOT}/logs_hidden_ifid_fullsketch_l2_50k50k"
STATUS_ROOT="${OUT_ROOT}/status_hidden_ifid_fullsketch_l2_50k50k"
...
run_gpu_worker() {
  ...
  for ((idx=gpu; idx<N; idx+=4)); do
    ...
    touch "${STATUS_ROOT}/${tag}.running"
    {
      ...
      echo "batch_size=${bs}"
      echo "reference=${SMALL_REF} validation=${SMALL_VAL} sketch_dim=${SKETCH_DIM} topk=${COARSE_TOPK} nn_metric=l2 coarse_metric=l2"
