    ...
    for nfe in nfes:
        ...
        for k, t in enumerate(times):
            ...
            if a.save_intermediate:
                im_m, im_l = decode_norm(vae, z_m, scale, bias), decode_norm(vae, z_l, scale, bias)
                pil_m, pil_l = tensor_to_pil(im_m), tensor_to_pil(im_l)
                tag = f"{next_t:g}".replace(".", "p")
                pil_m.save(out / f"nfe{nfe}_state_t{tag}_model.png")
                pil_l.save(out / f"nfe{nfe}_state_t{tag}_local.png")
                traj_sheet.append([
                    (pil_m, f"Model state @t={next_t:g}"),
                    (pil_l, f"Local state @t={next_t:g} | prev P={chosen_p}"),
                ])
        ...
        final_pil_m.save(out / f"nfe{nfe}_model_final.png")
        final_pil_l.save(out / f"nfe{nfe}_local_final.png")
        ...
        summary_sheet.append([
            ...
            (recon_pil, "VAE recon"),
            (final_pil_m, f"Model final NFE={nfe}"),
            (final_pil_l, f"Local final NFE={nfe} | P:{ptxt}"),
        ])

        if a.save_intermediate:
            traj_sheet.append([
                (final_pil_m, f"Model final xpred NFE={nfe}"),
                (final_pil_l, f"Local final xpred NFE={nfe} | P:{ptxt}"),
            ])
            make_contact_sheet(traj_sheet, out / f"trajectory_nfe{nfe}.png", cell=256)

        torch.cuda.empty_cache()

    summary_path = out / "comparison_nfe_2_4_8.png"
