#!/usr/bin/env python3
"""
V6-aligned hidden-interpolation FID with within-class nearest neighbours.

Everything except the NN search scope follows eval_sit_hidden_ifid_fullsketch_v6_a800.py:
    image -> VAE -> noisy SiT input -> layer-depth hidden h [B,T,D]
    -> choose the nearest reference hidden with the SAME ImageNet class
    -> exact cosine/L2 reranking on the complete unpooled hidden
    -> interpolate the selected complete hidden tensors
    -> mean-pool ONLY after NN selection/interpolation, for Frechet distance

Because a 50k ImageNet reference split contains roughly 50 examples per class,
all same-class references are searched directly. There is no pooled descriptor
and no global shortlist. This is the exact within-class counterpart of v6.

The random-neighbour sanity baseline is intentionally left global, exactly as
in v6, so the only experimental change is the NN search scope.

Requires eval_sit_hidden_ifid_100k50k_v3.py in the same directory.
The complete FP16 reference hidden bank can stay resident on an 80 GiB A800.
"""
...
def build_class_reference_indices(
    reference_labels,
) -> tuple[dict[int, np.ndarray], dict[str, float]]:
    """Build a CPU lookup from ImageNet class id to all reference indices."""
    if isinstance(reference_labels, torch.Tensor):
        labels_np = reference_labels.detach().cpu().numpy().astype(np.int64, copy=False)
    else:
        labels_np = np.asarray(reference_labels, dtype=np.int64)

    class_to_indices: dict[int, np.ndarray] = {}
    for class_id in np.unique(labels_np):
        indices = np.flatnonzero(labels_np == class_id).astype(np.int64, copy=False)
        class_to_indices[int(class_id)] = indices

    counts = np.asarray([len(v) for v in class_to_indices.values()], dtype=np.int64)
    if counts.size == 0:
        raise RuntimeError("Reference labels are empty")

    stats = {
        "reference_classes_present": float(counts.size),
        "reference_class_count_min": float(counts.min()),
        "reference_class_count_mean": float(counts.mean()),
        "reference_class_count_max": float(counts.max()),
    }
    print(
        "Within-class reference index: "
        f"classes={int(stats['reference_classes_present'])}, "
        f"count min/mean/max="
        f"{int(stats['reference_class_count_min'])}/"
        f"{stats['reference_class_count_mean']:.2f}/"
        f"{int(stats['reference_class_count_max'])}"
    )
    return class_to_indices, stats


def make_within_class_candidate_matrix(
    query_labels: torch.Tensor,
    class_to_indices: dict[int, np.ndarray],
) -> np.ndarray:
    """Return a padded [B,K] matrix containing every same-class reference.

    Padding repeats the first valid candidate. Repetition cannot change the
    argmax/argmin selected by the exact full-hidden reranker.
    """
    labels_np = query_labels.detach().cpu().numpy().astype(np.int64, copy=False)
    rows: list[np.ndarray] = []
    missing: list[int] = []

    for class_id in labels_np.tolist():
        indices = class_to_indices.get(int(class_id))
    ...
    if missing:
        raise RuntimeError(
            "No reference samples for query classes: "
            + ", ".join(map(str, sorted(set(missing))))
        )

    max_candidates = max(int(row.size) for row in rows)
    candidates = np.empty((len(rows), max_candidates), dtype=np.int64)
    for i, row in enumerate(rows):
        candidates[i, : row.size] = row
        if row.size < max_candidates:
            candidates[i, row.size :] = row[0]
    return candidates
...
def evaluate_queries(
    ...
    class_to_indices, class_stats = build_class_reference_indices(reference_labels)
    ...
    for images, labels, _names in val_loader:
        ...
        # Search ALL references with the same ImageNet class. At 50k reference
        # samples this is normally about 50 candidates per query, so no sketch
        # approximation is needed. Full [T,D] hidden tensors are used directly.
        candidates = make_within_class_candidate_matrix(labels, class_to_indices)
        nn_h, nn_indices = rerank_full_hidden_fast(
            query_h,
            candidates,
            reference_full,
            args.nn_metric,
        ...
        pbar.update(b)
        del (
            images, labels, z, query_h, candidates, nn_h,
    ...
    if same_class_total != n_val:
        raise RuntimeError(
            f"Within-class NN invariant failed: {same_class_total}/{n_val} "
            "selected neighbours have the query class"
        )
    ...
    return {
        "settings": {
            ...
            "nn_scope": "within_class",
            "retrieval_protocol": "all_same_class_references_exact_full_hidden_rerank",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--coarse-topk", type=int, default=64, help="Ignored for within-class exact search")
    p.add_argument("--sketch-dim", type=int, default=4096, help="Ignored for within-class exact search")
    p.add_argument("--sketch-seed", type=int, default=9187, help="Ignored for within-class exact search")
    p.add_argument("--sketch-build-batch", type=int, default=32, help="Ignored for within-class exact search")
...
def main() -> None:
    ...
    print("\n=== Hidden interpolation metrics (v6-aligned within-class full-hidden NN) ===")
