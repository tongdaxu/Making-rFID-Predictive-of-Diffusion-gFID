...
def main(args):
    ...
    if model_name not in SiT_models:
        ...
        if alt_model_name in SiT_models:
            print(f"[Model name alias] {model_name} -> {alt_model_name}")
    ...
    # Non-zero ranks do not need to wait while rank0 builds npz / computes FID.
    if rank != 0:
        dist.destroy_process_group()
        return

    # From here on, only rank0 runs post-processing.
    npz_path = create_npz_from_sample_folder(
        sample_folder_dir,
        args.num_fid_samples
    )
    print("create npz done:", npz_path)
