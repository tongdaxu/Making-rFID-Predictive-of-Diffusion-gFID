    ...
    for epoch in range(args.epochs):
        ...
        for raw_image, y in train_dataloader:
            ...
            with accelerator.autocast():
                ...
                bs, _, _, _ = processed_image.shape
                ...
                cos_sim = torch.mean(F.cosine_similarity(align_z.reshape(bs, -1), res_z.reshape(bs, -1)))
                cos_loss = torch.mean(F.relu(0.5 - F.cosine_similarity(align_z.reshape(bs, -1), res_z.reshape(bs, -1))))
                ...
                flow_loss = torch.mean(res_z**2)

                cos_grads = torch.autograd.grad(cos_loss, last_layer, retain_graph=True, allow_unused=True)[0]
                flow_grads = torch.autograd.grad(flow_loss, last_layer, retain_graph=True, allow_unused=True)[0]

                cos_weight = torch.norm(flow_grads) / (torch.norm(cos_grads) + 1e-4)
                cos_weight = torch.clamp(cos_weight, 0.0, 1e4).detach()

                # all_loss = flow_loss + 1.0 * cos_weight * cos_loss
                all_loss = flow_loss
                ...
                if accelerator.sync_gradients:
                    grad_norm = accelerator.clip_grad_norm_(
                        vae.parameters(), args.max_grad_norm
                    )
            ...
            if accelerator.sync_gradients:
                ...
                logs = {
                    "res_norm": accelerator.gather(res_norm).mean().detach().item(),
                    "cos_sim": accelerator.gather(cos_sim).mean().detach().item(),
                    "flow_loss": accelerator.gather(flow_loss).mean().detach().item(),
                    "psnr": accelerator.gather(psnr).mean().detach().item(),
                    "grad_norm": accelerator.gather(grad_norm)
                    .mean()
                    .detach()
                    .item(),
                    "epoch": epoch,
            ...
            if global_step % args.checkpointing_steps == 0 and global_step > 0:
                if accelerator.is_main_process:
                    ...
                    checkpoint = {
                        "flow": accelerator.unwrap_model(vae).flow.state_dict(),
