...
CUDA_VISIBLE_DEVICES=0,1,2,3 nohup accelerate launch --num_processes=4 train.py \
    --max-train-steps=400000 \
    --report-to="wandb" \
    --allow-tf32 \
    --mixed-precision="no" \
    --data-dir="/share/xutongda/ImageNeth5/data_256" \
    --output-dir="exps_unet" \
    --batch-size=128 \
    --path-type="linear" \
    --prediction="v" \
    --weighting="uniform" \
    --model="SiT/1" \
    --checkpointing-steps=200000 \
    --vae-config="./configs/PIX.yaml" \
    --bn-momentum=0.1 \
    --exp-name="sit-xl-pixel-400k" \
    &> sit-xl-pixel-400k.out &