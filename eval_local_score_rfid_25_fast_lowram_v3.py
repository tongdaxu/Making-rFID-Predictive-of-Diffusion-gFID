...
def parse_args():
    ...
    p.add_argument(
        "--bank-load-lock",
        type=str,
        default="/tmp/lsm_reference_bank_load.lock",
        help=(
            "Serialize loading large reference caches across the 8 model processes. "
            "This prevents cgroup page-cache/RSS spikes while still keeping all GPUs allocated."
        ),
    )
    p.add_argument(
        "--model-init-lock",
        type=str,
        default="/tmp/lsm_model_init.lock",
        help=(
            "Serialize VAE/checkpoint + Inception construction across model processes. "
            "This limits transient host-RAM spikes from loading several checkpoints at once."
        ),
    )
...
def exclusive_file_lock(lock_path: str):
    """
    Cross-process lock used only while streaming a reference cache from /share
    into GPU memory. All eight model processes can stay alive/on-GPU, but only
    one performs the host-heavy cache read at a time.
    """
    Path(lock_path).parent.mkdir(parents=True, exist_ok=True)
    with open(lock_path, "a+b") as lock_f:
        print(f"[bank-lock] waiting: {lock_path}", flush=True)
        fcntl.flock(lock_f.fileno(), fcntl.LOCK_EX)
        print(f"[bank-lock] acquired: {lock_path}", flush=True)
        try:
            yield
        finally:
            fcntl.flock(lock_f.fileno(), fcntl.LOCK_UN)
            print(f"[bank-lock] released: {lock_path}", flush=True)


def hard_trim_cpu():
    """
    Best-effort release of freed Python/PyTorch CPU heap pages back to the
    memory cgroup.  This is especially useful after checkpoint construction,
    where glibc may otherwise keep large freed arenas in the process RSS.
    """
    gc.collect()
    try:
        ...
        print(f"[malloc_trim] warning: {e!r}", flush=True)


def torch_load_cpu_drop_pagecache(path):
    """
    Load one torch cache chunk and explicitly tell Linux that the underlying
    file pages are no longer needed after the tensor has been copied to GPU.

    In a memory-cgroup environment, file page cache is charged to the cgroup
    even though `free -h` may show hundreds of GiB available on the host.
    """
    f = open(path, "rb")
    try:
...
def advise_drop_file_cache(f):
    try:
        ...
        # Advisory only; do not fail the experiment if the filesystem ignores it.
        print(f"[pagecache] fadvise warning: {e!r}", flush=True)
...
def load_normalized_reference_bank_gpu(
    ...
    with exclusive_file_lock(bank_load_lock):
        pbar = tqdm(cache_files, desc="load ref bank -> GPU")
        for i, meta in enumerate(pbar):
            pack, cache_f = torch_load_cpu_drop_pagecache(meta["path"])
            try:
                raw = pack["z"].to(
                ...
                # File bytes are no longer needed once the GPU copy is complete.
                advise_drop_file_cache(cache_f)
            finally:
                cache_f.close()

            del pack, raw, norm

            # Python/PyTorch CPU allocator cleanup. Do this periodically rather
            # than on every tiny chunk.
            if (i + 1) % 16 == 0:
                gc.collect()
...
def main(args):
    ...
    # Touch CUDA before waiting on the host-heavy init lock.  Each worker owns
    # its assigned GPU context immediately, while checkpoint construction is
    # serialized to avoid eight simultaneous CPU-RAM spikes.
    _cuda_context_hold = torch.empty(1, device=device)

    with exclusive_file_lock(args.model_init_lock):
        print(f"[model-init] building {args.exp_name}", flush=True)
        vae = instantiate_from_config(OmegaConf.load(args.vae_config)).to(device).eval()
        for p in vae.parameters():
            p.requires_grad_(False)

        # Probe latent layout while still under the init lock because some
        # wrappers lazily construct components on the first encode.
        ...
        # Inception construction can also transiently allocate host checkpoint
        # memory, so keep it in the same serialized section.
        ...
        hard_trim_cpu()
        print(f"[model-init] ready {args.exp_name}", flush=True)

    del _cuda_context_hold
    hard_trim_cpu()
    ...
    ref_bank_norm = load_normalized_reference_bank_gpu(
        ...
        bank_load_lock=args.bank_load_lock,
    )
    ...
    result = {
        ...
        "host_memory_mitigation": "serialized model/checkpoint init + malloc_trim + serialized reference-bank loading + POSIX_FADV_DONTNEED page-cache eviction + num_workers=0",
