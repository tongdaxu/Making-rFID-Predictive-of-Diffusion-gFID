...
def main(args):
    ...
    run_name = f"{exp_name}_step{int(args.train_steps):07d}_{args.time_mode}_{tag}_rfid_psnr_lpips"
    ...
    with torch.no_grad():
        for img, y, _name in pbar:
            ...
            # Match the previous proxy-rFID convention exactly.
            img01 = (img + 1.0) / 2.0
            src_feats.append(
                inception_features(inception, img01).float().cpu().numpy()
            )
            ...
            if any(k == "recon" for k, _ in settings):
                ...
                recon01 = (recon_raw + 1.0) / 2.0
                pred_feats["recon"].append(
                    inception_features(inception, recon01).float().cpu().numpy()
                )
            ...
            for kind, t in settings:
                ...
                pred01 = (pred_raw + 1.0) / 2.0
                pred_feats[key].append(
                    inception_features(inception, pred01).float().cpu().numpy()
                )

                del z0_hat_norm, z0_hat_raw, pred_raw, pred01

    # Save global-FID feature shards.
    np.save(
        out_dir / f"src_rank{rank:03d}.npy",
        np.concatenate(src_feats, axis=0),
    )
    for key, feats in pred_feats.items():
        np.save(
            out_dir / f"{key}_rank{rank:03d}.npy",
    ...
    if rank == 0:
        ...
        for kind, t in settings:
            ...
            pred_all = np.concatenate(
                [
                    np.load(out_dir / f"{key}_rank{r:03d}.npy")
                    for r in range(world_size)
                ],
                axis=0,
            )
            if pred_all.shape[0] != args.num_images:
                raise RuntimeError(
                    f"Expected {args.num_images} prediction features for {key}, "
                    f"got {pred_all.shape[0]}"
                )

            metrics[key] = {
                "rfid": fid_from_features(src_all, pred_all),
        ...
        result = {
            ...
            "rfid": {
                "inception_input": "float_[0,1]_matching_previous_proxy_rfid",
                "num_images": int(args.num_images),
            },
        ...
        for kind, t in settings:
            ...
            m = metrics[key]
            print(
                f"{key:16s}  rFID={m['rfid']:.6f}  "
                f"PSNR={m['psnr']:.6f}  LPIPS={m['lpips']:.6f}",
                flush=True,
            )
        ...
        for r in range(world_size):
            ...
            for kind, t in settings:
                key = setting_key(kind, t)
                (out_dir / f"{key}_rank{r:03d}.npy").unlink(missing_ok=True)
