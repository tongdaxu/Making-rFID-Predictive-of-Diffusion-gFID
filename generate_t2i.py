...
def main(args):
    ...
    if rank == 0:
        ...
        if os.path.isfile(fid_reference_file):
            ...
            fid = calculate_fid_given_paths(
                [args.fid_reference_folder, sample_folder_dir],
                batch_size=50,
                dims=2048,
                device="cuda",
                num_workers=8,
        ...
        print("fid={0:.2f}".format(fid))

        if args.fid_reference_folder != "" and os.path.isfile(fid_reference_file):
            from torch_fidelity import calculate_metrics
            other = calculate_metrics(
                input1=sample_folder_dir, 
                input2=args.fid_reference_folder, 
                cuda=True, 
                isc=True, 
                fid=False, 
                kid=True, 
                ...
                samples_resize_and_crop=256,
            )
            print("is={0:.2f}".format(other["inception_score_mean"]))
            print("precision={0:.4f}".format(other["precision"]))
            print("recall={0:.4f}".format(other["recall"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fid-reference-folder", type=str, default="")
