...
EVAL_PY="${EVAL_PY:-$REPO/eval_proxy_dinocos_v2.py}"
...
SETTINGS="${SETTINGS:-ode:0.8,sde:0.8,ode:0.4,sde:0.4,ode:0.2,sde:0.2}"
DINO_ENC_TYPE="${DINO_ENC_TYPE:-dinov2-vit-b}"
...
import sys
print(sys.argv[1].replace(":", "").replace(",", "__").replace(".", "p"))
PY
)

OUT_ROOT="${OUT_ROOT:-/share/xutongda/REPA-E/exps_interpolate/proxy_dinocos_${BACKBONE}_${SETTINGS_TAG}}"
...
echo 'model,exp_path,setting,dino_cls_cos,dino_cls_dist,dino_patch_cos,dino_patch_dist,dino_global_cos,dino_global_dist,status' > "$SUMMARY_LONG"
...
for raw in sys.argv[1].split(','):
    ...
    if raw=="recon":
        ...
        k,t=raw.split(":",1)
        item=(k.strip().lower(),float(t))
    q=key(*item)
    if q not in seen:
        seen.add(q)
        out.append(q.replace(".", "p"))
print("__".join(out))
PY
)
...
for spec in "${MODELS[@]}"; do
  ...
  if [[ $VRC -ne 0 ]]; then
    ...
    echo "$LABEL,$EXP,,,,,,,,VALIDATION_FAILED" >> "$SUMMARY_LONG"
    continue
  fi
  ...
  RUN_DIR="$OUT_ROOT/${EXP_NAME}_step$(printf '%07d' "$TRAIN_STEPS")_${TIME_MODE}_${EVAL_TAG}_dinocos"
...
for setting, m in d["metrics"].items():
    print(
        f"{label},{exp},{setting},"
        f"{m['dino_cls_cos']},{m['dino_cls_dist']},"
        f"{m['dino_patch_cos']},{m['dino_patch_dist']},"
        f"{m['dino_global_cos']},{m['dino_global_dist']},DONE"
    )
PY
  else
...
for s in settings:
    header += [
        ...
        f"{s}_dino_cls_dist",
        f"{s}_dino_patch_cos",
        f"{s}_dino_patch_dist",
        f"{s}_dino_global_cos",
        f"{s}_dino_global_dist",
    ]
