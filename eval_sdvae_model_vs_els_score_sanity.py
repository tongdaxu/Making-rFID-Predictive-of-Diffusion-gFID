...
def parse_args():
    ...
    p.add_argument(
        "--sanity-only",
        action="store_true",
        help="Run the P=1 ELS spatial-permutation invariance sanity test and exit.",
    )
    p.add_argument(
        "--sanity-time",
        type=float,
        default=0.4,
        help="Noise time used by the sanity test.",
    )
    p.add_argument(
        "--sanity-ref-size",
        type=int,
        default=1000,
        help="Use only the first N reference latents for the sanity test.",
    )
    p.add_argument(
        "--sanity-shift",
        type=int,
        ...
        default=[7, 11],
        metavar=("DY", "DX"),
        help="Spatial roll used as one negative-control transform.",
    )
...
def compare_tensors(a: torch.Tensor, b: torch.Tensor):
    ...
    return {
        ...
        "rmse": float(d.square().mean().sqrt().item()),
        "rel_mse": float((d.square().mean() / denom).item()),
        "cos": float(flat_cos(a, b)),
    }


@torch.no_grad()
def run_p1_spatial_invariance_sanity(
    *,
    bank: torch.Tensor,
    z_t: torch.Tensor,
    alpha,
    ...
    ELS candidate set = every spatial vector from every reference image.
    Therefore any spatial permutation of the reference bank must leave the
    ELS posterior mean unchanged (up to floating-point reduction error).

    LS is co-located, so the same spatial permutation should change LS.
    """
    n = min(int(ref_size), int(bank.shape[0]))
    if n <= 0:
        raise ValueError(f"sanity-ref-size must be > 0, got {ref_size}")

    b0 = bank[:n].contiguous()
    _, _, h, w = b0.shape

    dy, dx = int(shift[0]), int(shift[1])
    b_roll = torch.roll(b0, shifts=(dy, dx), dims=(2, 3))

    # Stronger control: apply one fixed random permutation to the H*W spatial
    # locations of every reference image.  For P=1 this preserves the exact
    # ELS candidate multiset but destroys co-location for LS.
    gen = torch.Generator(device=b0.device)
    gen.manual_seed(int(seed) + 99173)
    perm = torch.randperm(h * w, generator=gen, device=b0.device)
    b_perm = (
        b0.flatten(2)[:, :, perm]
        .reshape_as(b0)
        .contiguous()
    ...
    def ls_p1(b):
        _ideal, by_p = analytic_posteriors(
            bank=b,
            z_t=z_t,
            alpha=alpha,
            sigma=sigma,
    ...
    print("\n" + "=" * 72)
    print("P=1 ELS SPATIAL-INVARIANCE SANITY TEST")
    print(f"references : {n}")
    print(f"latent grid: {h}x{w}")
    print(f"roll       : dy={dy}, dx={dx}")
    print("=" * 72)

    print("[1/6] LS(original)")
    ls0 = ls_p1(b0)
    print("[2/6] LS(rolled)")
    ls_roll = ls_p1(b_roll)
    print("[3/6] LS(permuted)")
    ls_perm = ls_p1(b_perm)

    print("[4/6] ELS(original)")
    els0 = els_p1(b0)
    print("[5/6] ELS(rolled)")
    els_roll = els_p1(b_roll)
    print("[6/6] ELS(permuted)")
    els_perm = els_p1(b_perm)

    result = {
        "reference_count": n,
        "latent_grid": [h, w],
        "roll": [dy, dx],
        "ls_roll_vs_original": compare_tensors(ls0, ls_roll),
        "ls_perm_vs_original": compare_tensors(ls0, ls_perm),
        "els_roll_vs_original": compare_tensors(els0, els_roll),
        "els_perm_vs_original": compare_tensors(els0, els_perm),
        "ls_vs_els_original": compare_tensors(ls0, els0),
    }
    ...
    for key, val in result.items():
        if isinstance(val, dict):
            print(
                f"{key:24s} "
                f"max_abs={val['max_abs']:.3e}  "
                f"rel_mse={val['rel_mse']:.3e}  "
                f"cos={val['cos']:.9f}"
            )

    # We deliberately avoid an excessively strict fixed tolerance because the
    # ELS implementation uses FP32 streamed reductions.  The signature we want
    # is orders-of-magnitude separation:
    #   ELS transformed ~= ELS original
    #   LS transformed  != LS original
    eps = 1e-30
    roll_ratio = (
        result["ls_roll_vs_original"]["rel_mse"] + eps
    ) / (
        result["els_roll_vs_original"]["rel_mse"] + eps
    )
    perm_ratio = (
        result["ls_perm_vs_original"]["rel_mse"] + eps
    ) / (
        result["els_perm_vs_original"]["rel_mse"] + eps
    )
    result["ls_to_els_error_ratio_roll"] = float(roll_ratio)
    result["ls_to_els_error_ratio_perm"] = float(perm_ratio)

    print(
        f"\nLS/ELS error ratio (roll) = {roll_ratio:.3e}\n"
        f"LS/ELS error ratio (perm) = {perm_ratio:.3e}"
    )
    print(
        "\nExpected signature:\n"
        "  ELS roll/perm: cos ~= 1, rel_mse ~= numerical noise\n"
        "  LS  roll/perm: clearly non-zero rel_mse\n"
        "If that happens, the P=1 ELS implementation really is pooling all "
        "reference spatial positions."
    )
    return result
...
def main(args):
    ...
    if args.sanity_only:
        ...
        sanity = run_p1_spatial_invariance_sanity(
            bank=bank,
            z_t=z_t,
            alpha=alpha,
            sigma=sigma,
            ref_chunk=args.els_ref_chunk,
            ref_size=args.sanity_ref_size,
            shift=args.sanity_shift,
            seed=args.seed,
        )
        sanity.update({
            "t_raw": t_raw,
        ...
        path = out / "sanity_p1_spatial_invariance.json"
