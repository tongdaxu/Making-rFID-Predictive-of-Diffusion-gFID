        ...
        for raw_image, y in train_dataloader:
            ...
            with accelerator.autocast():
                ...
                ema_outputs_ref = ema(
                    ...
                    loss_kwargs=loss_kwargs,
                    time_input=torch.cat([torch.rand_like(time_in) * 0.2, time_in], dim=0),
                    noises=torch.cat([torch.randn_like(sit_outputs["noises"]), torch.randn_like(sit_outputs["noises"])], dim=0),
                    return_feat=True,
                )
                feat1, feat2 = torch.chunk(ema_outputs_ref["fs_tilde"][0], 2, dim=0)
                feat1, feat2 = torch.nn.functional.normalize(feat1, dim=-1), torch.nn.functional.normalize(feat2, dim=-1)
                proj_loss = torch.mean(mean_flat(-(feat1 * feat2).sum(dim=-1)))
