...
def create_npz_from_sample_folder(sample_dir, num=50_000):
    ...
    print(f"Saved .npz file to {npz_path} [shape={samples.shape}].", flush=True)
    return npz_path
...
def main(args):
    ...
    seed = (args.global_seed + rank) * world_size // 2
    ...
    if args.exp_path is None or args.train_steps is None:
        if rank == 0:
            print(
                "The `exp_path` or `train_steps` is not provided, setting defaults.",
                flush=True,
            )
    ...
    if len(fake_z.shape) == 3:
        ...
        raise ValueError(f"Unexpected fake_z shape: {fake_z.shape}")

    tshift = math.sqrt(float(fake_z.numel()) / 4096.0)
    print("using time shift {0:.4f}".format(tshift), flush=True)

    block_kwargs = {"fused_attn": config.fused_attn, "qk_norm": config.qk_norm}
    model_name = config.model

    if model_name not in SiT_models:
        alt_model_name = model_name.replace("SiT_", "SiT-")
        if alt_model_name in SiT_models:
            print(f"[Model name alias] {model_name} -> {alt_model_name}", flush=True)
            model_name = alt_model_name
        else:
            raise KeyError(
                f"Unknown model name: {config.model}. "
                f"Available models: {list(SiT_models.keys())}"
            )
    ...
    if vae_1d:
        ...
        latents_bias = (
            state_dict["ema"]["bn.running_mean"]
            .view(1, 1, in_channels)
            .to(device)
        )
    else:
    ...
    sample_folder_dir = (
        f"{args.sample_dir}/"
        f"{exp_name}_{train_step_str}_cfg"
        f"{args.cfg_scale}-{args.guidance_low}-{args.guidance_high}"
    )
    ...
    if rank == 0:
        print(sample_folder_dir, flush=True)
        print(f"Total number of images that will be sampled: {total_samples}", flush=True)
        print(f"SiT Parameters: {sum(p.numel() for p in model.parameters()):,}", flush=True)
    ...
    for _ in pbar:
        ...
        with torch.no_grad():
            if args.mode == "sde":
                samples, samples_alt = (
                    euler_maruyama_sampler(**sampling_kwargs).to(torch.float32),
                    None,
                )
            elif args.mode == "ode":
                samples, samples_alt = (
                    euler_sampler(**sampling_kwargs).to(torch.float32),
                    None,
                )
            elif args.mode == "edict":
                samples, samples_2, samples_hist = edict_sampler(**sampling_kwargs)
                samples = samples.to(torch.float32)
                samples_2 = samples_2.to(torch.float32)

                sampling_kwargs["latents"] = samples
                inv_samples, inv_samples_2, inverse_hist = edict_inverter(
                    **sampling_kwargs
                )

                mse_z = torch.mean((inv_samples - z) ** 2)
                print(mse_z, mse_z / torch.mean(z**2), flush=True)

                samples_hist = samples_hist[::-1]
                for i in range(len(samples_hist)):
                    print(
                        i,
                        torch.mean((samples_hist[i] - inverse_hist[i]) ** 2).item(),
                        flush=True,
                    )

                save_tensor_image(
                    F.pixel_shuffle(torch.clamp(inv_samples, -1, 1), 16),
                    "z1.png",
                )
                save_tensor_image(
                    F.pixel_shuffle(torch.clamp(inv_samples_2, -1, 1), 16),
                    "z2.png",
                )

                sampling_kwargs["latents"] = inv_samples.to(torch.float32)
                samples_alt, samples_alt_2, _ = edict_sampler(**sampling_kwargs)
                samples_alt = samples_alt.to(torch.float32)
                samples_alt_2 = samples_alt_2.to(torch.float32)
            else:
            ...
            if samples_alt is not None:
                ...
                print(
                    "edict psnr: {0:.2f}".format(torch.mean(pred_psnr).item()),
                    flush=True,
                )
    ...
    # Make sure all processes have finished saving samples.
    dist.barrier()

    # Non-zero ranks do not need to wait while rank0 computes FID / builds npz.
    if rank != 0:
    ...
    print(f"Calculating FID with {fid_num} number of samples", flush=True)

    fid = calculate_fid_given_paths(
        [args.fid_reference_file, sample_folder_dir],
    ...
    if args.create_npz:
        ...
        print("create npz done:", npz_path, flush=True)

    if not args.keep_samples:
        shutil.rmtree(sample_folder_dir)

    # Keep the final effective line as fid=..., so shell scripts can detect success.
    print("fid=", fid, flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=True,
        help="By default, use TF32 matmuls.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Batch size used when calculating FID.",
    )
    parser.add_argument(
        "--fid-num-workers",
        type=int,
        default=8,
        help="Number of workers used when calculating FID.",
    )
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Whether to build .npz from generated PNG samples after FID.",
    )
    parser.add_argument(
        "--keep-samples",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether to keep the PNG sample folder after FID.",
    )
