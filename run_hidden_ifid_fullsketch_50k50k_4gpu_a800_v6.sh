#!/usr/bin/env bash
# A800-80GB mode: keep each model's complete 50k FP16 reference hidden bank on its GPU.
...
PY_SCRIPT="${REPO}/eval_sit_hidden_ifid_fullsketch_v6_a800.py"
...
RESULT_ROOT="${OUT_ROOT}/hidden_ifid_fullsketch_results_50k50k"
LOG_ROOT="${OUT_ROOT}/logs_hidden_ifid_fullsketch_50k50k"
STATUS_ROOT="${OUT_ROOT}/status_hidden_ifid_fullsketch_50k50k"
...
run_gpu_worker() {
  ...
  for ((idx=gpu; idx<N; idx+=4)); do
    ...
    touch "${STATUS_ROOT}/${tag}.running"
    {
      ...
      echo "batch_size=${bs}"
      echo "reference=${SMALL_REF} validation=${SMALL_VAL} sketch_dim=${SKETCH_DIM} topk=${COARSE_TOPK}"
