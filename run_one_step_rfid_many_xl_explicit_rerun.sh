...
BASE_PORT="${BASE_PORT:-32600}"

# IMPORTANT: this is a NEW output tree, separate from the previous XL run.
OUT_ROOT="${OUT_ROOT:-/share/xutongda/REPA-E/exps_interpolate/one_step_rfid_many_xl_shifted_raw_rerun}"
LOG_ROOT="${LOG_ROOT:-$OUT_ROOT/logs}"
SUMMARY="$OUT_ROOT/summary.csv"
MANIFEST="$OUT_ROOT/manifest.tsv"

# Explicit mapping to the XL baseline experiments already audited.
MODELS=(
...
if not model_norm.startswith("SIT-XL/"):
...
for spec in "${MODELS[@]}"; do
  ...
  if [[ -s "$METRICS" ]]; then
    echo "[$LABEL] existing result found in RERUN folder; skip compute"
    STATUS="DONE"
  else
