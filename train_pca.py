    ...
    for epoch in range(args.epochs):
        ...
        for raw_image in train_dataloader:
            ...
            with accelerator.autocast():
                ...
                _, info = vae(processed_image)
                xhat = info["xhat"]
                xhat_partial = info["xhat_partial"]
                xhat_test = info["xhat_test"]

                rec_loss = torch.abs(processed_image - xhat_partial)
                p_loss = loss_fn_vgg(processed_image, xhat_partial)
                all_loss = torch.mean(rec_loss + p_loss)
                ...
                with torch.no_grad():
                    psnr_ref = torch.mean(get_psnr(processed_image, xhat, zero_mean=True, integer=True))
                    psnr_ts = torch.mean(get_psnr(processed_image, xhat_test, zero_mean=True, integer=True))
                    psnr_tr = torch.mean(get_psnr(processed_image, xhat_partial, zero_mean=True, integer=True))
                    x_r0, x_f0 = accelerator.unwrap_model(vae).test(processed_image, 0)
                    x_r1, x_f1 = accelerator.unwrap_model(vae).test(processed_image, 1)
                    x_r2, x_f2 = accelerator.unwrap_model(vae).test(processed_image, 2)
                    psnr_r0 = torch.mean(get_psnr(processed_image, x_r0, zero_mean=True, integer=True))
                    psnr_r1 = torch.mean(get_psnr(processed_image, x_r1, zero_mean=True, integer=True))
                    psnr_r2 = torch.mean(get_psnr(processed_image, x_r2, zero_mean=True, integer=True))
                    psnr_f0 = torch.mean(get_psnr(processed_image, x_f0, zero_mean=True, integer=True))
                    psnr_f1 = torch.mean(get_psnr(processed_image, x_f1, zero_mean=True, integer=True))
                    psnr_f2 = torch.mean(get_psnr(processed_image, x_f2, zero_mean=True, integer=True))
            ...
            if accelerator.sync_gradients:
                ...
                logs = {
                    "psnr_ref": accelerator.gather(psnr_ref).mean().detach().item(),
                    "psnr_ts": accelerator.gather(psnr_ts).mean().detach().item(),
                    "psnr_tr": accelerator.gather(psnr_tr).mean().detach().item(),
                    "psnr_r0": accelerator.gather(psnr_r0).mean().detach().item(),
                    "psnr_r1": accelerator.gather(psnr_r1).mean().detach().item(),
                    "psnr_r2": accelerator.gather(psnr_r2).mean().detach().item(),
                    "psnr_f0": accelerator.gather(psnr_f0).mean().detach().item(),
                    "psnr_f1": accelerator.gather(psnr_f1).mean().detach().item(),
                    "psnr_f2": accelerator.gather(psnr_f2).mean().detach().item(),
                    "epoch": epoch,
                }
            ...
            if global_step % 500 == 0:
                if accelerator.is_main_process:
                    accelerator.log({"source": wandb.Image(array2grid((processed_image+1.0)/2.0))}, step=global_step)
                    accelerator.log({"x_f0": wandb.Image(array2grid((x_f0+1.0)/2.0))}, step=global_step)
                    accelerator.log({"x_f1": wandb.Image(array2grid((x_f1+1.0)/2.0))}, step=global_step)
                    accelerator.log({"x_f2": wandb.Image(array2grid((x_f2+1.0)/2.0))}, step=global_step)
                    accelerator.log({"xhat": wandb.Image(array2grid((xhat+1.0)/2.0))}, step=global_step)
