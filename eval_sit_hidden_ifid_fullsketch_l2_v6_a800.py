#!/usr/bin/env python3
"""
V6-aligned hidden-interpolation FID with L2 full-hidden retrieval.

This keeps the original v6 engineering pipeline:
    image -> VAE -> noisy SiT input -> layer-depth hidden h [B,T,D]
    -> deterministic signed-hash sketch built from ALL T*D coordinates
       (no spatial/global pooling)
    -> L2 top-k retrieval in sketch space
    -> exact squared-L2 reranking on the complete unpooled hidden
    -> linear interpolation on the complete unpooled hidden
    -> mean-pool only for Frechet-distance evaluation

The complete FP16 reference hidden bank is kept resident on an 80 GiB A800.
This is still a top-k approximate global search, exactly like v6; it is not
the exhaustive-all-reference v7 protocol.
...
class TorchSketchIndex:
    """Exact squared-L2 search over compressed full-hidden sketches."""
    ...
    def __init__(
        ...
        self.device = device
        # Use float32 for ranking. The 50k x 4096 matrix is only about 0.76 GiB.
        ...
        self.reference_sq = (ref * ref).sum(dim=1).contiguous()
        gib = (
            self.reference_t.numel() * self.reference_t.element_size()
            + self.reference_sq.numel() * self.reference_sq.element_size()
        ) / (1024 ** 3)
        print(
            "Full-hidden sketch index: exact squared-L2 in sketch space; "
    ...
    def search_tensor(self, query_sketch: torch.Tensor, k: int) -> torch.Tensor:
        """Return L2 shortlist indices directly on the index GPU."""
        q = query_sketch.float()
        q_sq = (q * q).sum(dim=1, keepdim=True)
        dot = q @ self.reference_t
        dist = q_sq + self.reference_sq.unsqueeze(0) - 2.0 * dot
...
def sketch_paths(cache_dir: str, sketch_dim: int, sketch_seed: int) -> Tuple[str, str]:
    stem = f"reference_fullsketch_l2_raw_d{sketch_dim}_seed{sketch_seed}"
    return (
...
def build_or_load_reference_sketch(
    ...
    expected = {
        ...
        "protocol": "signed_hash_of_full_hidden_no_pool_l2_raw_sketch",
        "sketch_metric": "squared_l2",
    }
...
def evaluate_queries(
    ...
    full_l2_sum = 0.0
    pooled_l2_sum = 0.0
    query_to_nn_mse_sum = 0.0
    query_to_random_mse_sum = 0.0
    nn_full_norm_sum = 0.0
    random_full_norm_sum = 0.0
    ...
    for images, labels, _names in val_loader:
        ...
        # NN search uses a compressed representation of ALL hidden coordinates.
        # There is no mean/max pooling before or during candidate retrieval.
        query_sketch = sketcher.transform(query_h)
        if isinstance(reference_full, torch.Tensor):
            candidates = sketch_index.search_tensor(query_sketch, args.coarse_topk)
        else:
            candidates = sketch_index.search(query_sketch, args.coarse_topk)
        ...
        # Diagnostics only; they do not change NN, interpolation, pooling, or FID.
        query_to_nn_mse_sum += float(
            ((query_h - nn_h) ** 2).flatten(1).mean(dim=1).sum().item()
        )
        query_to_random_mse_sum += float(
            ((query_h - random_h) ** 2).flatten(1).mean(dim=1).sum().item()
        )
        nn_full_norm_sum += float(nn_h.flatten(1).norm(dim=1).sum().item())
        random_full_norm_sum += float(random_h.flatten(1).norm(dim=1).sum().item())
        ...
        pbar.update(b)
        del (
            images, labels, z, query_h, query_sketch, candidates, nn_h,
    ...
    half = n_val // 2
    selected_all = np.concatenate(selected_indices).astype(np.int64, copy=False)
    unique_nn, nn_counts = np.unique(selected_all, return_counts=True)
    ...
    metrics = {
        ...
        "query_to_nn_full_mse": query_to_nn_mse_sum / float(n_val),
        "query_to_random_full_mse": query_to_random_mse_sum / float(n_val),
        "nn_hidden_mean_full_l2": nn_full_norm_sum / float(n_val),
        "random_hidden_mean_full_l2": random_full_norm_sum / float(n_val),
        "nn_unique_count": float(unique_nn.size),
        "nn_unique_fraction": float(unique_nn.size) / float(n_val),
        "nn_max_reuse_count": float(nn_counts.max()) if nn_counts.size else 0.0,
    ...
    return {
        "settings": {
            ...
            "retrieval_protocol": "full_hidden_signed_hash_l2_topk_then_exact_full_hidden_l2_rerank",
            "coarse_representation": "unnormalized_signed_hash_of_all_TxD_hidden_coordinates_no_pool",
            "coarse_metric": "squared_l2",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--nn-metric", choices=["l2"], default="l2")
...
def main() -> None:
    ...
    reference_sketch = build_or_load_reference_sketch(
        args, cache, sketcher, device
    )
    sketch_index = TorchSketchIndex(reference_sketch, device)
    ...
    print("\n=== Hidden interpolation metrics (v6-aligned L2 full-hidden sketch retrieval) ===")
