...
EVAL_PY="${EVAL_PY:-$REPO/eval_proxy_psnr_lpips.py}"
...
SETTINGS="${SETTINGS:-direct:0.1,direct:0.4,direct:0.8,ode:0.8,sde:0.8,ode:0.4,sde:0.4,ode:0.2,sde:0.2}"
# Optional example:
...
OUT_ROOT="${OUT_ROOT:-/share/xutongda/REPA-E/exps_interpolate/proxy_psnr_lpips_${BACKBONE}_${SETTINGS_TAG}}"
...
echo 'model,exp_path,setting,psnr,lpips,status' > "$SUMMARY_LONG"
...
for spec in "${MODELS[@]}"; do
  ...
  RUN_DIR="$OUT_ROOT/${EXP_NAME}_step$(printf '%07d' "$TRAIN_STEPS")_${TIME_MODE}_${EVAL_TAG}"
...
for setting, m in d["metrics"].items():
    print(f"{label},{exp},{setting},{m['psnr']},{m['lpips']},DONE")
PY
  else
...
with open(long_path, newline='') as f:
    for r in csv.DictReader(f):
        ...
        rows[model][r['setting']]=(r['psnr'],r['lpips'])

header=['model']
for s in settings:
    header += [f'{s}_psnr', f'{s}_lpips']
