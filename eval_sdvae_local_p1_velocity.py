...
def parse_args():
    ...
    p.add_argument("--trajectory-steps", type=int, default=8,
                   help="Number of Euler denoising steps from trajectory-start to 0.")
...
def run_velocity_trajectory(
    ...
    traj_dir = Path(out_dir) / "trajectory_t0p8_velocity"
    ...
    # A compact trajectory sheet is useful for deciding whether failure is
    # already present along the path or appears only near the endpoint.
    labels = [f"t={t:.2f}" for t in times]
    sheet_rows = [[im] for im in state_pils]
    contact_sheet(
        sheet_rows,
        ["Velocity trajectory state"],
        labels,
        traj_dir / "trajectory_states.png",
        cell=256,
    )
    ...
    return {
        ...
        "trajectory_sheet": str(traj_dir / "trajectory_states.png"),
    }
